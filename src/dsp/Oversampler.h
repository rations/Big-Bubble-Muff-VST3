// BigBubbleMuff — 4x half-band polyphase-IIR oversampler (mono).
// Copyright (C) 2026  BigBubbleMuff contributors. SPDX-License-Identifier: MIT
//
// Two cascaded 2x half-band stages wrapped around the nonlinear clipping core, the
// same arrangement and four filter specifications the engine has always used.
//
...
// (and independent state): the up and down specifications are asymmetric.
...
public:
  ...
  inline void processUp(float x, float &even, float &odd) noexcept {
    ...
    for (std::size_t i = 0; i < UpDirect; ++i)
      d = at(upDirectState_, i).process(d, at(upDirect_, i));
    float q = x;
    for (std::size_t i = 0; i < UpDelayed; ++i)
      q = at(upDelayedState_, i).process(q, at(upDelayed_, i));
    even = d;
    odd = q;
  }
  ...
  inline float processDown(float even, float odd) noexcept {
    ...
    for (std::size_t i = 0; i < DownDirect; ++i)
      d = at(downDirectState_, i).process(d, at(downDirect_, i));
    float q = downPrevOdd_;
    for (std::size_t i = 0; i < DownDelayed; ++i)
      q = at(downDelayedState_, i).process(q, at(downDelayed_, i));
    downPrevOdd_ = odd;
    return 0.5f * (d + q);
  }

private:
  UpDirectCoeffs upDirect_;
  UpDelayedCoeffs upDelayed_;
  DownDirectCoeffs downDirect_;
  DownDelayedCoeffs downDelayed_;
...
public:
  ...
  // Latency in base-rate samples: each stage contributes the DC delay of its up and down
  // filters divided by the cumulative oversampling factor at that stage.
